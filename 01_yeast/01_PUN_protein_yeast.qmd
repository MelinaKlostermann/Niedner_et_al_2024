---
title: "Find and characterise PUN proteins in yeast"
...
out <- "/Users/melinaklostermann/Documents/projects/PUN-proteins/PUN_code/Report1_clean/"
...
# import sequences
protein_seqs <- readAAStringSet("/Users/melinaklostermann/Documents/projects/PUN-proteins/PUN_code/Uniprot_protein_seq_per_gene/SCerevisiae/UP000002311_559292.fasta.gz") 
...
# save plot
ggsave(paste0(out, "PUN_motifs_per_protein_SC_", Sys.Date(), ".pdf"), width = 4, height = 6, units = "cm")
...
ggsave(p, file = paste0(out, "sizes_of_PUN_clusters_SC_", Sys.Date(), ".pdf"), width = 8, height = 10, units = "cm")
...
ggrepel::geom_label_repel(data = top_10[1:5,], aes(x = id, y = n_PUN_norm, label = protein_name), max.overlaps = 10)+
...
ggsave(p, file = paste0(out, "PUN_proteins_per_length_SC_", Sys.Date(), ".pdf"), width = 6, height = 6, units = "cm")
```
...
ggsave(paste0(out, "PUN_motifs_per_protein_zoom_in_SC_", Sys.Date(), ".pdf"), width = 4, height = 6, units = "cm")

```

Number of proteins with >= 3 PUN motifs within `r my_dist` AA: `r seqs %>% subset(smallest_adj_dist < my_dist) %>% nrow(.)`.
...
loc1_motif_seqs <-  AAStringSet(all_motifs %>% subset (protein_name == "LOC1") %>% .$PUN_motifs)
...
ggsave(paste0(out, "Logo_LOC1_PUN_motifs_", Sys.Date(), ".pdf"), width = 6, height = 6, units = "cm")
...
for(i in 1:nrow(top_10)){
  ...
  ggplot(df, aes(xmin = pos, xmax = pos_end, ymin = 0, ymax = 1))+
    ...
    labs(x = "")
  ggsave(paste0(out, "Schema", s$protein_name, "_", Sys.Date(), ".pdf"), width = 4, height = 2.5, units = "cm")
}
...
ggsave(p, file = paste0(out, "Comp_disorder_SC_", Sys.Date(), ".pdf"), width = 4, height = 8, units = "cm")
...
entrez_ids <- mapIds(org.Sc.sgd.db, keys = seqs$protein_id, column = "ENTREZID", keytype = "UNIPROT")
entrez_ids_bg <- mapIds(org.Sc.sgd.db, keys = all_seqs$protein_id, column = "ENTREZID", keytype = "UNIPROT")
...
ggsave(file = paste0(out, "GO_SC_", Sys.Date(), ".pdf"), width = 14, height = 10, units = "cm")
...
# Get all GO terms
go <- go_sets(org.Sc.sgd.db, "UNIPROT", go = c("GO", "GOID"), ontology = "CC")
...
ggsave(paste0(out, "Nucleus_pie_SC.pdf"), width = 5, height = 5, units = "cm")
```

# Number PUN proteins in Riobosome biogenesis

```{r}
go <- go_sets(org.Sc.sgd.db, "UNIPROT", go = c("GO", "GOID"), ontology = "BP")
...
ribosome_set_id <- "GO:0042254"
ribosome_set <- es %>% subset(set == ribosome_set_id)

# Overlap sets with PUN proteins
prot_ribosome <- seqs %>% subset(protein_id %in% ribosome_set$element)
...
xlsx::write.xlsx2(x = as.data.frame(seqs), file = paste0(out, "PUN_proteins_SC", Sys.Date(), ".xlsx"))
```
