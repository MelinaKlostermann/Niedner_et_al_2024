---
title: "Characterisation of PUN proteins in SPombe"
...
out <- "/Users/melinaklostermann/Documents/projects/PUN-proteins/PUN_code/Report8-SPombe/"
...
protein_seqs <- readAAStringSet("/Users/melinaklostermann/Documents/projects/PUN-proteins/PUN_code/Uniprot_protein_seq_per_gene/Spombe /UP000002485_284812.fasta.gz") 
...
Uniport ID are tranfered to entrz IDs with annotation downloaded from https://ftp.ensemblgenomes.ebi.ac.uk/pub/fungi/release-58/gff3/schizosaccharomyces_pombe/
...
library(biomaRt)
mart <- useMart(biomart="fungi_mart", dataset="spombe_eg_gene", host="https://fungi.ensembl.org")

my_attributes <- c("uniprotswissprot", "name_1006", "go_id")
info_biomart_go <- getBM(attributes=my_attributes,
                        mart = mart, useCache = F)

saveRDS(info_biomart_go, file = paste0(out, "biomart_info_SP.rds"))
```
...
entrez_ids <- left_join(seqs, info_biomart_go, by = c(protein_id = "uniprotswissprot")) 

entrez_ids_bg <- left_join(all_seqs, info_biomart_go, by = c(protein_id = "uniprotswissprot")) 

term2gene <- info_biomart_go[,c("go_id", "uniprotswissprot" )]
term2name <- info_biomart_go[,c("go_id", "name_1006" )]


# Calculate GO enrichment for the three Ontologies separately
go_enrichment <- enricher(
  entrez_ids$protein_id,
  pvalueCutoff = 0.05,
  pAdjustMethod = "BH",
  universe = entrez_ids_bg$protein_id,
  minGSSize = 10,
  maxGSSize = 500,
  qvalueCutoff = 1,
  TERM2GENE = term2gene,
...
#----------------------
info_biomart_go <- readRDS(paste0(out, "biomart_info_SP.rds"))


nucleus_set <- info_biomart_go %>% subset(name_1006 == "nucleus")
nucleolus_set <- info_biomart_go %>% subset(name_1006 == "nucleolus")

# Overlap sets with PUN proteins
prot_nucleolus <- seqs %>% subset(protein_id %in% nucleolus_set$uniprotswissprot)
prot_other_nucleus <- seqs %>% subset((protein_id %in% nucleus_set$uniprotswissprot) & !(protein_id %in% nucleolus_set$uniprotswissprot)) 
prot_other <- seqs %>% subset(!(protein_id %in% nucleus_set$uniprotswissprot) & !(protein_id %in% nucleolus_set$uniprotswissprot)) 
